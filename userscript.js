...
(function () {
  ...
  var h = this.height(),
    w = this.width(),
    r =
      this.isStatic || this instanceof TemplateSlotMorph ||
    ...
    InputSlotMorph.prototype.fixLayout = function () {
      ...
      if (this.selectedBlock) {
        ...
        if (
          this.squareStrings
            ? !(!this.isNumeric && (!this.isReadOnly || this.isStatic))
            : !(this instanceof TextSlotMorph) || !this.isStatic
        ) {
          width = Math.max(
            contents.width() +
  ...
  function setSymbolImages() {
    ...
    SymbolMorph.prototype.drawImage = function (ctx, image) {
      ...
      var prW =
          document.getElementById("world").getAttribute("width") /
          document.getElementById("world").width,
        prH =
          document.getElementById("world").getAttribute("height") /
          document.getElementById("world").height,
        w = this.width(),
        h = this.height(),
        ow = image.width,
        oh = image.height;
    ...
    SymbolMorph.prototype.renderSymbolArrowUp = function (ctx, color) {
      ...
      this.drawImage(
        ctx,
        this[color.eq(BLACK) ? "arrowImageBlack" : "arrowImage"],
      );
      return;
      ctx.fillStyle = color.toString();
      ctx.lineWidth = l * 2;
