...
(function () {
  ...
  // $*!% like this is patently ugly and slows Snap down. Tnx!
    ...
    if ((!proc.isRunning() && !proc.errorFlag) || proc.isDead) {
      if (proc.topBlock instanceof BlockMorph) {
        ...
        if (count) {
          ...
          if (!proc.isGenericCondition || proc.hasFiredGenericCondition) {
            if (
...
Process.prototype.reportAssociativeBool = function (block, baseOp, short) {
  ...
  if (inputs.length < 1) {
    if (inline) {
      ...
      // tests is an ArgLabelMorph
      this.pushContext(tests.argMorph(), outer);
    }
  } else if (inputs.length === 1) {
    if (acc) {
      // inline - acc has been initialized
      if (inputs[0] === short) {
        if (this.flashContext()) {
          return;
        }
    ...
    inputs.push(this.hyper(baseOp, inputs.pop(), inputs.pop()));
    acc.pc += 1;
  }
};
...
Process.prototype.evaluateMultiSlot = function (multiSlot, argCount) {
  ...
  if (multiSlot.bindingID) {
    ...
    if (argCount > inputs.length) {
      ...
      if (
        ["%t", "%upvar"].includes(multiSlot.slotSpec) && // var template
      ...
      } else if (multiSlot.slotSpec instanceof Array) {
        // input group
        if (
          multiSlot.slotSpec.includes("%t") ||
          ...
          for (i = 0; i < inputs.length; i += 1) {
            if (
              ...
              this.context.outerContext.variables.addVar(inputs[i]);
            }
          }
        }
        if (
          ...
          !["%receive", "%send", "%survey"].includes(multiSlot.elementSpec)
        ) {
          // format the inputs as 2D table, unless it's a "built-in"
          // group, e.g. for broadcast, scene changes etc.
          ans = new List(inputs);
          inputs = this.reportNumbers(1, ans.length())
            .reshape(new List([0, multiSlot.slotSpec.length]))
            .map((indices) => ans.query(indices))
            .itemsArray();
        }
      }
...
Process.prototype.isAutoLambda = function (inputSlot) {
  ...
  return (
    inputSlot.constructor === CommandSlotMorph ||
...
Process.prototype.reportSelf = function (trgt) {
  ...
  if (!this.isAtomic && this.context.expression.parent?.selector === "doRun") {
...
Process.prototype.reportContinuation = function (trgt) {
  ...
  if (frame) {
    ...
    cont = new Context(null, "popContext");
    cont.isContinuation = true;
  }
  return cont;
};
    ...
    if (slot instanceof InputSlotMorph) {
      ...
      value.itemsArray().forEach((item, i) => {
        if (subslots[i] instanceof InputSlotMorph) {
          subslots[i].setContents(isNil(item) ? "" : item.toString());
...
Process.prototype.evaluateCustomBlock = function () {
  if (
    this.context.expression instanceof CustomHatBlockMorph &&
    !this.context.accumulator
  ) {
...
Process.prototype.doInsertInList = function (element, index, list) {
  ...
  if (parseFloat(idx) !== +idx) {
    // treat as alphanumerical index
  ...
  switch (option) {
    ...
    case "uniques":
      ...
      if (
        ...
        return this.reportListAttribute("distribution", list).columns().at(1);
      }
    ...
    case "distribution":
      ...
      if (list.canBeJSON()) {
        ...
        return list
          ...
          .distribution()
          .map((row) => {
            return new List([this.restoreColor(row.at(1)), row.at(2)]);
          });
      }
...
Process.prototype.reportUniqueValues = function (list) {
  ...
  if (this.context.accumulator.idx === list.length()) {
    this.returnValueToParentContext(new List(this.context.accumulator.target));
    return;
  }
...
Process.prototype.reportDistribution = function (list) {
  ...
  if (this.context.accumulator.idx === list.length()) {
    this.returnValueToParentContext(
      new List(
        this.context.accumulator.target
          .sort((a, b) => b[1] - a[1])
...
Process.prototype.reportSorted = function (data) {
  return new List(
    data
      .itemsArray()
      .slice()
      .sort((a, b) => (this.reportIsBefore(a, b) ? -1 : 1)),
  );
};
...
Process.prototype.reportIsBefore = function (a, b) {
  ...
  switch (typeA) {
    ...
    case "predicate":
      return (
        a.expression.abstractBlockSpec() < b.expression.abstractBlockSpec()
      );
    case "costume":
    case "sound":
    case "sprite":
...
Process.prototype.reportConcatenatedLists = function (lists) {
  ...
  for (rowIdx = 1; rowIdx <= rows; rowIdx += 1) {
    ...
    if (row instanceof List) {
      ...
      // append scalar as new list item
      result.push(row);
    }
  }
  return new List(result);
  ...
  if (!this.context.startTime) {
    if (thisObj) {
      if (this.inputOption(name) === "center") {
        ...
        if (thatObj) {
          ((endX = thatObj.xPosition()), (endY = thatObj.yPosition()));
        }
      }
    }
...
Process.prototype.playSound = function (name) {
  if (name instanceof List) {
    ...
    return this.doPlaySoundAtRate(
      name,
      this.blockReceiver().parentThatIsA(StageMorph).microphone?.audioContext
        ?.sampleRate || 44100,
    );
  }
    ...
    sound =
      name instanceof Sound
        ...
        : typeof name === "number"
          ...
          : detect(this.blockReceiver().sounds.asArray(), (s) =>
      ...
      soundName instanceof Sound
        ...
        : typeof soundName === "number"
          ...
          : soundName instanceof List
            ...
            : detect(this.blockReceiver().sounds.asArray(), (s) =>
                snapEquals(s.name, soundName.toString()),
              ),
...
Process.prototype.doBroadcast = function (message, options) {
  ...
  if (msg !== "") {
          ...
          if (block.selector === "receiveMessage") {
            ...
            if (varName) {
              ...
              if (choice instanceof Array && choice[0].indexOf("any") === 0) {
                varFrame.addVar(
                  varName,
                  payload !== "" ? new List([message, payload]) : message,
                );
              } else {
    ...
    (stage.messageCallbacks[msg] || []).forEach(
      (callback) => callback(payload), // for a particular message
    );
  }
  return procs;
};
...
Process.prototype.assertType = function (thing, typeString, ...exempt) {
  ...
  if (exempt.length && contains(exempt, thing)) {
    return thing;
  }
...
Process.prototype.hyperChangeBy = function (data, delta) {
  ...
  var match = Math.min(this.reportQuickRank(data), this.reportQuickRank(delta));
  this.hyperMutate(this.reportBasicSum, data, delta, match, match);
};
...
Process.prototype.packCoordinates = function (list) {
  ...
  return this.isCoordinate(list)
    ? new Variable(list)
    : list.map((each) =>
        each instanceof List ? this.packCoordinates(each) : each,
      );
};
...
Process.prototype.reportSequentialComparison = function (op, items, notHyper) {
  ...
  for (i = 1; i < len; i += 1) {
    ...
    if (isNil(result)) {
      ...
      result = this.hyper((a, b) => a && b, result, step);
    }
  }
  return result;
};
...
Process.prototype.reportLessThanOrEquals = function (a, b) {
  return this.hyper((a, b) => !this.reportBasicGreaterThan(a, b), a, b);
};
...
Process.prototype.reportGreaterThanOrEquals = function (a, b) {
  return this.hyper((a, b) => !this.reportBasicLessThan(a, b), a, b);
};
...
Process.prototype.reportMonadic = function (fname, n) {
  return this.hyper((num) => this.reportBasicMonadic(fname, num), n);
};
...
Process.prototype.reportLetter = function (idx, string) {
  return this.hyper((ix, str) => this.reportBasicLetter(ix, str), idx, string);
};
...
Process.prototype.reportStringSize = function (data) {
  return this.hyper(
    (str) =>
      isString(str)
        ? this.safeStringArray(str).length
        : parseFloat(str) === +str
          ? str.toString().length
          : 0,
    data,
  ...
  switch (this.inputOption(delimiter)) {
    ...
    default:
      ...
      if (this.isCaseInsensitive) {
        del = new RegExp(del.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&"), "ig");
      }
  }
...
Process.prototype.toBlockSyntax = function (list) {
  ...
  head = list.at(1);
  if (snapEquals(head, "items")) {
    // reserved token for data
    data = this.toInputSyntax(list.cdr());
    return list.cons(data.length(), data);
  }
  return this.variadify(
    list.cons(
      head instanceof List
        ? this.toBlockSyntax(head)
...
Process.prototype.blockMatching = function (string) {
  ...
  var pal = this.context.accumulator || this.reportGet("blocks"),
    ...
    i;
  if (pal) {
    // when bootstrapping the palette at first launch it can be empty
  ...
  return (
    SpriteMorph.prototype.blockForSelector(this.blockAlias(string)) ||
    SpriteMorph.prototype.variableBlock(" ")
  ).reify();
};
...
Process.prototype.variadify = function (list) {
  ...
  slot = ring.expression.inputs().find((any) => any instanceof MultiArgMorph);
  if (slot) {
    ...
    if (list.at(idx + 1) === ":") {
      ...
      syntax.add(
        new List([list.cons(list.length() - idx, new List(items.slice(idx)))]),
      );
    }
    return syntax;
  }
...
Process.prototype.blockAlias = function (string) {
  return snapEquals(string, "get")
    ? "reportGetVar"
    : Object.keys(SpriteMorph.prototype.blocks).find((key) =>
        snapEquals(SpriteMorph.prototype.blocks[key]?.code, string),
      ) || string;
};

Process.prototype.selectorAlias = function (string) {
  return string === "reportGetVar"
    ? "get"
...
Process.prototype.toTextSyntax = function (list) {
  ...
  return syn.cons(
    head instanceof List
      ? this.toTextSyntax(head)
      : head === Symbol.for("items")
        ? "items" // reserved token for data
...
Process.prototype.devariadify = function (list) {
  ...
  if (slot) {
    ...
    if (inps.length + slots.length < list.length()) {
      ...
      arr
        .slice(idx + inps.length, list.length())
        .forEach((each) => syntax.add(each));
      return syntax;
    }
  }
  return list;
...
Process.prototype.getOtherObject = function (name, thisObj, stageObj) {
  ...
  if (stage) {
    ...
    thatObj = detect(stage.children, (morph) => snapEquals(morph.name, name));
    if (!thatObj) {
...
Process.prototype.castColor = function (color) {
    ...
    if (color instanceof Costume) {
      return this.castColor(
        color
          .rasterized()
          .pixels()
          .reshape(new List([color.height(), color.width(), 4])),
      );
    }
  }
  if (color instanceof List) {
    first = clr.at(1);
    if (
      (first instanceof Color || first instanceof Costume) &&
      this.enableHyperOps
    ) {
      return clr.map((each) => this.castColor(each));
    }
...
Process.prototype.reportNewColor = function (hsbt) {
  ...
  if (this.reportQuickRank(hsbt) > 1) {
    // hyper-monadicized
...
Process.prototype.objectTouchingObject = function (thisObj, name) {
  ...
  var those, stage, box, coord;

  if (this.inputOption(name) === "mouse-pointer") {
    coord = thisObj.world().hand.position();
    if (
      thisObj.bounds.containsPoint(coord) &&
...
Process.prototype.reportAspect = function (aspect, location, noRerender) {
  ...
  if (this.enableHyperOps) {
    if (location instanceof List && !this.isCoordinate(location)) {
      return location.map((each) => this.reportAspect(aspect, each, true));
    }
  }
  ...
  if (target === "myself") {
    ...
    if (choice === "sprites") {
      ...
      clr = world.getGlobalPixelColor(world.hand.position(), noRerender);
    }
  } else if (target instanceof List) {
    point = new Point(
    ...
    if (choice === "sprites") {
      ...
      clr = world.getGlobalPixelColor(point, noRerender);
    }
  } else {
    if (!target) {
      return;
...
Process.prototype.reportBasicAttributeOf = function (attribute, name) {
  ...
  if (thisObj) {
      ...
      switch (this.inputOption(attribute)) {
        ...
        case "extent":
          if (thatObj instanceof StageMorph) {
            return new List([thatObj.dimensions.x, thatObj.dimensions.y]);
          }
    ...
    if (this.inputOption(attribute) === "input names") {
      return new List(thatObj instanceof Context ? thatObj.inputs.slice() : []);
    }
  }
  return "";
};
...
Process.prototype.doSet = function (attribute, value) {
  ...
  switch (name) {
    ...
    case "my scripts":
      ...
      value.map((each) =>
        this.assertType(each, ["command", "reporter", "predicate", "hat"]),
      );
...
Process.prototype.reportNewCostumeStretched = function (name, xP, yP) {
  ...
  if (!cst) {
    throw new Error("expecting a costume\nbut getting none");
    // return new Costume();
  }
  ...
  if (shp instanceof List && shp.at(2) > 0) {
    ...
    if (shp.at(3) < 1) {
      return result
        ...
        .columns()
        .at(1)
        .reshape(new List([yStretch, xStretch]));
    }
...
Process.prototype.costumeNamed = function (name) {
  ...
  return detect(this.blockReceiver().costumes.asArray(), (c) =>
    snapEquals(c.name, name.toString()),
  );
};
  ...
  if (value !== undefined) {
    ...
    if (
      ...
      if (value instanceof List) {
        ...
        anchor.showBubble(value, this.exportResult, this.receiver);
      }
    }
  }
};
...
Process.prototype.reportBasicBlockAttribute = function (attribute, block) {
  ...
  switch (choice) {
    ...
    case "definition":
      ...
      if (
        body.expression &&
        body.expression.selector === "doReport" &&
    ...
    case "type":
      return (
        ["command", "reporter", "predicate", "hat"].indexOf(
        ...
        expr
          ...
          .map((each) =>
            each instanceof ReporterBlockMorph
              ? each.getSlotSpec()
              : each instanceof MultiArgMorph && each.slotSpec instanceof Array
                ? each.slotSpec
                : each.getSpec(),
          ),
    ...
    case "defaults":
      ...
      if (expr.isCustomBlock) {
        ...
        def.declarations.forEach((value) => {
          if (
            (value[0] || "").toString().startsWith("%mult") ||
        ...
        if (
          expr.inputs().slice(-1)[0] instanceof MultiArgMorph &&
      ...
      if (expr.isCustomBlock) {
        ...
        expr.inputs().forEach((slot) => {
          if (slot instanceof ReporterBlockMorph) {
            slot = SyntaxElementMorph.prototype.labelPart(slot.getSlotSpec());
          }
          slots.add(
            slot instanceof InputSlotMorph
              ? isString(slot.choices)
                ? "§_" + slot.choices
                : CustomBlockDefinition.prototype.decodeChoices(slot.choices)
              : "",
          );
        });
      }
    ...
    case "editables":
      ...
      if (expr.isCustomBlock) {
        ...
        expr.inputs().forEach((slot) => {
          ...
          slots.add(slot instanceof InputSlotMorph ? !slot.isReadOnly : false);
        });
      }
      return slots;
    case "replaceables":
    ...
    case "separators":
      ...
      if (expr.isCustomBlock) {
        ...
        expr.inputs().forEach((slot) => {
          ...
          slots.add(slot instanceof MultiArgMorph ? slot.infix : "");
        });
      }
      return slots;
    case "collapses":
      ...
      if (expr.isCustomBlock) {
        ...
        expr.inputs().forEach((slot) => {
          ...
          slots.add(slot instanceof MultiArgMorph ? slot.collapse : "");
        });
      }
      return slots;
    case "expands":
      ...
      if (expr.isCustomBlock) {
        ...
        expr.inputs().forEach((slot) => {
          ...
          if (slot instanceof MultiArgMorph) {
            data =
              slot.labelText instanceof Array
                ? new List(
                    slot.labelText.map((item) => item.replaceAll("\n", " ")),
                  )
    ...
    case "initial slots":
      ...
      if (expr.isCustomBlock) {
        ...
        expr.inputs().forEach((slot) => {
          ...
          slots.add(slot instanceof MultiArgMorph ? slot.initialSlots : "");
        });
      }
      return slots;
    case "min slots":
      ...
      if (expr.isCustomBlock) {
        ...
        expr.inputs().forEach((slot) => {
          ...
          slots.add(slot instanceof MultiArgMorph ? +slot.minInputs : "");
        });
      }
      return slots;
    case "max slots":
      ...
      if (expr.isCustomBlock) {
        ...
        expr.inputs().forEach((slot) => {
          ...
          slots.add(slot instanceof MultiArgMorph ? +slot.maxInputs || 0 : "");
        });
      }
      return slots;
    case "translations":
...
Process.prototype.slotSpec = function (num) {
  ...
  if (
    num > 100 &&
    ["loop", "scriptVars", "receive", "send", "elseif"].includes(spec) &&
    num > 100
  ) {
...
Process.prototype.doSetBlockAttribute = function (attribute, block, val) {
  ...
  function labelFromList(list) {
    return list
      .map((each) => (each instanceof List ? each.asWords() : each))
      .asTXT()
      .replaceAll("\n", " $nl ");
  }
  ...
  switch (choice) {
    ...
    case "primitive":
      ...
      if (prim && prim.selector === "doPrimitive" && prim.nextBlock()) {
        ...
        def.setPrimitive(val ? prim.inputs()[1].contents().text || null : null);
      }
      break;
    case "category":
      this.assertType(val, ["number", "text"]);
      if (this.reportTypeOf(val) === "text") {
        idx = SpriteMorph.prototype
          .allCategories()
    ...
    case "translations":
      ...
      loc = {};
      val.map((row) => (loc[row.at(1).toString()] = row.at(2).toString()));
...
Process.prototype.compileBlockReferences = function (context, varName) {
  ...
  if (
    ...
    if (context.expression instanceof ReporterBlockMorph) {
      ...
      report.replaceInput(report.inputs()[0], context.expression.fullCopy());
  ...
  if (context.expression instanceof BlockMorph) {
    context.expression.forAllChildren((morph) => {
      var ref;
      if (morph.selector === "reportGetVar" && morph.blockSpec === varName) {
...
Process.prototype.doDeleteBlock = function (context) {
  ...
  if (def.isBootstrapped()) {
    ...
    if (def.isGlobal) {
      ...
      if (method) {
        rcvr
          .allDependentInvocationsOf(method.blockSpec)
  ...
  this.names(true).forEach((vName) => {
    ...
    if (names.includes(vName)) {
      ...
      if (v.isTransient) {
        ...
        if (
          ["text", "number", "Boolean"].includes(typ) ||
...
VariableFrame.prototype.merge = function (otherFrame) {
  ...
  otherFrame
    .names(true)
    .forEach((vName) => (this.vars[vName] = otherFrame.vars[vName]));
};
...
VariableFrame.prototype.changeVar = function (name, delta, sender) {
  ...
  if (frame) {
    if (frame instanceof List) {
      ...
      newValue = isNaN(parseFloat(value))
        ? delta
    ...
    newValue =
      !(value instanceof List) && isNaN(parseFloat(value))
        ? delta
        : Process.prototype.reportSum(value, delta);
    if (
      sender instanceof SpriteMorph &&
...
JSCompiler.prototype.gensymForVar = function (varName, argIndex) {
  ...
  var gensym = this.getGensym(varName),
    oldArgIndex;
  if (gensym == null) {
...
JSCompiler.prototype.compileInfix = function (operator, inputs) {
  return inputs
    .map((each) => this.compileInput(each))
    .join(" " + operator + " ");
};
