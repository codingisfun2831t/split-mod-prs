...
List.prototype.wrapIndex = function (num) {
  var idx = Math.round(+num || 0),
    mod = (a, b) => ((+a % +b) + +b) % +b;
  return this.enableWrapping ? mod(idx - 1, this.length()) + 1 : idx;
};
...
List.prototype.lookup = function (key, ifNone = "") {
  ...
  if (parseFloat(key) === +key) {
    // treat as numerical index
    return this.at(key);
  }
  rec = this.itemsArray().find(
    (elem) =>
      elem instanceof List && elem.length() > 0 && snapEquals(elem.at(1), key),
  );
  if (rec) {
...
List.prototype.forget = function (key) {
  ...
  var idx = 0,
    ...
    query = (rec) =>
      snapEquals(rec, key) ||
      (rec instanceof List && rec.length() === 2 && snapEquals(rec.at(1), key));
...
List.prototype.fillDimensionsFor = function (dimensions, leafCount) {
  ...
  if (
    ...
    product = dimensions
      .itemsArray()
      .reduce((a, b) => Math.max(a, 1) * Math.max(b, 1));
    factor = Math.ceil(leafCount / Math.max(product, 1));
    return dimensions.map((each) => (each ? each : already++ ? factor : 1));
  }
  return dimensions;
};
...
List.prototype.distribution = function () {
  ...
  return new List(
    [...dict].sort((a, b) => b[1] - a[1]).map((pair) => new List(pair)),
  );
};
...
List.prototype.asJSON = function () {
  ...
  function objectify(list) {
    ...
    return items.map((element) =>
      element instanceof List ? objectify(element) : numberize(element),
    );
  }
...
function ListWatcherMorph(list, parentCell, noScale) {
  this.init(list, parentCell);
}

ListWatcherMorph.prototype.init = function (list, parentCell, noScale) {
  var myself = this,
    readOnly;
  ...
  this.rounding = noScale ? 9 : SyntaxElementMorph.prototype.rounding;
  this.scale = noScale ? 1 : SyntaxElementMorph.prototype.scale;
  ...
  this.setExtent(
    new Point(80, 70).multiplyBy(this.scale),
  );
  this.add(this.label);
  this.add(this.frame);
  if (!readOnly) {
